...
function(a) {
	...
	if (b.DEFAULTS = {
		...
		function f() {
			...
			if (d || (d = b.node.blockParent(c), d && "LI" != d.tagName && (d = null)), d) if (b.node.isBlock(d) && b.node.isEmpty(d))"LI" == d.tagName && d.parentNode.firstElementChild == d ? a(d).append('<span class="fr-marker"></span>') : a(d).replaceWith('<span class="fr-marker"></span>');
		...
		function n(c) {
			...
			if (b.opts.htmlAllowComments || c.nodeType != Node.COMMENT_NODE) if (c.tagName && c.tagName.match(H)) c.parentNode.removeChild(c);
		...
		function t(a, c) {
			...
			else for (e = b.el.getElementsByTagName("br"), d = 0; d < e.length; d++) r(e[d], c)
		}
		function u() {
		...
		function j() {
			...
			if (p.indexOf('id="docs-internal-guid') >= 0 && (p = p.replace(/^.* id="docs-internal-guid[^>]*>(.*)<\/b>.*$/, "$1"), t = !0), j && !b.wordPaste && (p = p.replace(/^\n*/g, "").replace(/^ /g, ""), 0 === p.indexOf("<colgroup>") && (p = "<table>" + p + "</table>"), p = g(p), p = m(p)), b.opts.pastePlain && (p = h(p)), l = b.events.chainTrigger("paste.afterCleanup", p), "string" == typeof l && (p = l), "" !== p) {
				...
				if (b.opts.enter == a.FE.ENTER_BR) for (d = u.querySelectorAll("p, div"), c = d.length - 1; c >= 0; c--) e = d[c], e.outerHTML = e.innerHTML + (e.nextSibling && !b.node.isEmpty(e) ? "<br>" : "");
				else if (b.opts.enter == a.FE.ENTER_DIV) for (d = u.getElementsByTagName("p"), c = d.length - 1; c >= 0; c--) e = d[c], e.outerHTML = "<div>" + e.innerHTML + "</div>";
